...
export class AuthorizationController {
  ...
  async crearAutorizacionPermanente(
    ...
    let propietarioId = req.user?.personaId;
    
    if (req.user?.role === UserRole.ADMIN) {
      const vehiculo = await this.registryPort.findVehiculoById(dto.vehiculoId);
      if (!vehiculo) {
        throw new BadRequestException(`Vehículo '${dto.vehiculoId}' no encontrado`);
      }
      propietarioId = vehiculo.propietarioPersonaId;
  ...
  async crearPermisoTemporal(@Request() req: any, @Body() dto: CrearPermisoTemporalDto) {
    ...
    return this.authorizationService.crearPermisoTemporal(dto, propietarioId);
  }
