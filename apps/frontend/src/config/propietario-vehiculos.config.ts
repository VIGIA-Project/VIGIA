...
export const MIS_VEHICULOS_COPY = {
  ...
  emptyState: {
    ...
    description: 'Registra tu primer vehículo para comenzar a gestionar permisos y accesos.',
  },
} as const;
...
export const VEHICLE_CARD_COPY = {
  ...
  personasLabel: (n: number) => (n === 0 ? 'Sin personas asignadas' : n === 1 ? 'Personas: 1' : `Personas: ${n}`),
  personasSinBiometriaTooltip: (n: number) => `${n} persona${n === 1 ? '' : 's'} sin biometría`,
...
export const MOCK_ACCESOS_RECIENTES: AccesoRecienteEvento[] = [
  { id: 'acc-1', placa: 'PCB-1234', title: 'Acceso autorizado · Acceso Norte', subtitle: 'Validación biométrica exitosa', timestamp: 'hace 2h', severity: 'success' },
  { id: 'acc-2', placa: 'GHI-8821', title: 'Alerta operativa · Acceso Sur', subtitle: 'Intento de acceso fuera de horario habitual', timestamp: 'hace 5h', severity: 'warning' },
  { id: 'acc-3', placa: 'PCB-1234', title: 'Salida registrada · Acceso Norte', subtitle: 'Salida exitosa', timestamp: 'ayer', severity: 'info' },
];
...
export const loadOnboardingVehiculo = (): PropietarioVehiculo | null => {
  try {
    ...
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { placa: string; marca: string; modelo: string; color: string; anio: number };
...
export const buildInitialVehiculos = (): PropietarioVehiculo[] => {
  ...
  const withoutDuplicate = MOCK_VEHICULOS.filter((v) => v.placa !== onboardingVehiculo.placa);
