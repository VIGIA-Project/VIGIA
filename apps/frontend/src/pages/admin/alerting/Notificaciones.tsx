...
// Derivamos notificaciones de las alertas reales (simulación)
const mapAlertToNotificacion = (alert: Alert, i: number): Notificacion => ({
  id: i + 1,
  alerta: alert.alertTitle.substring(0, 15).toUpperCase(),
  canal: i % 2 === 0 ? "EMAIL" : "PUSH",
  destinatario: i % 2 === 0 ? "admin@uce.edu.ec" : "App Guardia",
  estado: alert.status === "ACTIVE" ? "PENDIENTE" : "ENTREGADA",
...
export default function Notificaciones() {
  ...
  const rows: Notificacion[] = alerts.slice(0, 6).map(mapAlertToNotificacion);
