...
const rows: Cuenta[] = personas.map((p, i) => ({
  id: p.id,
  identificacion: p.cedula,
  nombre: p.nombre,
  correo: `${p.nombre.split(" ")[0].toLowerCase()}@uce.edu.ec`,
  rol: i === 0 ? "Administrador" : i % 2 === 0 ? "Guardia" : "Docente",
  estado: p.estado === "ACTIVA" ? "ACTIVO" : "DESACTIVADO",
  ultimoLogin: p.autorizadoDesde,
}));
...
export default function CuentasList() {
  ...
  const filteredRows = filterRol
    ? rows.filter((r) => r.rol === filterRol)
    : rows;
