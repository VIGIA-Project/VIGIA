...
export default function PersonasList() {
  ...
  const [personasData, setPersonasData] = useState<PersonaAutorizada[]>(() =>
    loadPersonas(),
  );
  ...
  const [editingPersona, setEditingPersona] =
    useState<PersonaAutorizada | null>(null);

  const rows: Persona[] = personasData.map((p) => ({
    id: p.id,
    identificacion: p.cedula,
    nombre: p.nombre,
    correo: p.telefono ? `${p.telefono}@placeholder.local` : "",
    rol: p.relacion,
    estado: p.estado === "ACTIVA" ? "ACTIVO" : "REVOCADA",
    tieneCuenta: p.biometria === "COMPLETADA",
    esAgregado: p.tipo === "familia",
    ultimoLogin: null,
  }));
  ...
  return (
    <Box>
      ...
      <DataTable
        ...
        rowActions={(row) => {
          ...
          actions.push({
            ...
            label: "Editar",
            onClick: () => {
              const p = personasData.find((x) => x.id === row.id);
              if (p) {
                setEditingPersona(p);
                setOpenUnificadoModal(true);
              }
          ...
          actions.push({
            ...
            onClick: () =>
              setConfirm({
                ...
                action: () => {
                  const updated = personasData.filter((x) => x.id !== row.id);
      ...
      <RegistrarUnificadoModal
        ...
        onSuccess={(data) => {
          if (data.id || editingPersona) {
            const idToUse = data.id ?? editingPersona?.id;
            const updated = personasData.map((p) =>
              p.id === idToUse
                ? {
                    ...p,
                    nombre:
                      ...
                      p.nombre,
                    cedula: data.identificacion || p.cedula,
                    relacion: data.rol || p.relacion,
            ...
            const nueva: PersonaAutorizada = {
              id: `per-${Date.now()}`,
              nombre:
                `${data.nombres || ""} ${data.apellidos || ""}`.trim() ||
                "Sin nombre",
              cedula: data.identificacion || "",
              relacion: data.rol || "Agregado",
              tipo: "familia",
              biometria: "PENDIENTE",
              estado: "ACTIVA",
              telefono: data.telefono || undefined,
              autorizadoDesde: new Date().toLocaleDateString(),
            };
            const updated = [nueva, ...personasData];
            savePersonas(updated);
            setPersonasData(updated);
          }
          setEditingPersona(null);
