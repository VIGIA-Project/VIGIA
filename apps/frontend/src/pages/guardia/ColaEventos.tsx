...
interface Evento {
  id: number; placa: string; tiempo: string; minutos: number;
  movimiento: Movimiento; hora: string; motivo: Motivo;
  descripcion: string; criticidad: Criticidad;
  vehiculo: string; propietario: string; score?: number;
}

const EVENTOS: Evento[] = [
  { id:1, placa:'PCB-1234', tiempo:'hace 1 min', minutos:1, movimiento:'SALIDA', hora:'13:24:15', motivo:'CONDUCTOR_NO_AUTORIZADO', descripcion:'La persona capturada no coincide con ningún conductor autorizado vigente. Riesgo de retiro no autorizado.', criticidad:'ALTA', vehiculo:'Toyota Corolla Blanco', propietario:'Carlos Mendoza' },
  ...
  { id:4, placa:'MNB-4455', tiempo:'hace 7 min', minutos:7, movimiento:'SALIDA', hora:'13:17:22', motivo:'CONDUCTOR_PRESENTA_PASE', descripcion:'El conductor presenta un código de pase rápido para validación.', criticidad:'BAJA', vehiculo:'Nissan Versa Plata', propietario:'Ana García' },
];
...
const critLabel: Record<Criticidad,string> = {
  ALTA: '⚠ Alta criticidad', MEDIA: 'Media criticidad', BAJA: 'Baja criticidad',
};
...
export const ColaEventosPage: React.FC = () => {
  ...
  const [filtroMov, setFiltroMov] = useState(estadoFiltro || 'Todos');
  ...
  const btnFiltro = (label: string, active: boolean, onClick: ()=>void, color='#0D5CCF') => (
    <button key={label} onClick={onClick} style={{
      padding:'6px 16px', borderRadius:20, fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'Inter,sans-serif',
    ...
    <DashboardTemplate rol="GUARD" pageTitle="Cola de eventos">
      <Box sx={{ fontFamily:'Inter,sans-serif' }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16 }}>
          <div>
            <div style={{ fontFamily:'"Exo 2",sans-serif', fontSize:20, fontWeight:700, color:'#0F172A' }}>Cola de eventos</div>
            <div style={{ fontSize:13, color:'#6B7280', marginTop:3 }}>Acceso Norte · <strong style={{ color:'#C62828' }}>{visibles.length} eventos pendientes</strong></div>
          </div>
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', padding:'14px 18px', marginBottom:18, display:'flex', flexDirection:'column', gap:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            ...
            {['Todos','ENTRADA','SALIDA'].map(o => btnFiltro(o, filtroMov===o, ()=>setFiltroMov(o), o==='ENTRADA'?'#1565C0':o==='SALIDA'?'#4527A0':'#0D5CCF'))}
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            <span style={{ fontSize:11, fontWeight:600, color:'#94A3B8', minWidth:80 }}>Criticidad:</span>
            {['Todos','ALTA','MEDIA','BAJA'].map(o => btnFiltro(o, filtroCrit===o, ()=>setFiltroCrit(o), o==='ALTA'?'#C62828':o==='MEDIA'?'#F2851F':'#0D5CCF'))}
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
              ...
              <div key={ev.id} style={{ borderRadius:14, overflow:'hidden', border:`1px solid ${cc.border}`, background:cc.cardBg, boxShadow:'0 3px 14px rgba(10,47,134,0.08)' }}>
                ...
                <div style={{ padding:'18px 22px', display:'flex', alignItems:'center', gap:18 }}>
                  <div style={{ flexShrink:0 }}>
                    <div style={{ fontFamily:'"Exo 2",sans-serif', fontSize:20, fontWeight:800, color:'#0A2F86', background:'#EEF2FF', padding:'6px 16px', borderRadius:8, border:'1px solid #C7D2FE', letterSpacing:2, marginBottom:8 }}>{ev.placa}</div>
                    <span style={{ fontSize:10, fontWeight:600, padding:'3px 10px', borderRadius:20, background:mc.bg, color:mc.text }}>{ev.movimiento}</span>
                  </div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:8, flexShrink:0, minWidth:178 }}>
                    {ev.motivo === 'CONDUCTOR_NO_AUTORIZADO' && <>
                      <button onClick={() => setModalDenegar(ev)} style={{ padding:'10px 16px', borderRadius:9, background:'#C62828', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>✕ Denegar salida</button>
                      <button onClick={() => navigate('/guardia/revision-manual', { state:{ eventoId:ev.id, placa:ev.placa } })} style={{ padding:'10px 16px', borderRadius:9, background:'#0D5CCF', color:'#fff', fontSize:12, fontWeight:600, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>ℹ Ver detalle</button>
                    </>}
                    {ev.motivo === 'VEHICULO_NO_REGISTRADO' && <>
                      <button onClick={() => navigate('/guardia/registro-invitado', { state:{ eventoId:ev.id, placa:ev.placa } })} style={{ padding:'10px 16px', borderRadius:9, background:'#0D5CCF', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>👤 Registrar invitado</button>
                      <button onClick={() => setModalDenegar(ev)} style={{ padding:'10px 16px', borderRadius:9, background:'#C62828', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>✕ Denegar ingreso</button>
                      <button onClick={() => navigate('/guardia/revision-manual', { state:{ eventoId:ev.id } })} style={{ padding:'9px 16px', borderRadius:9, background:'#F8FAFC', color:'#374151', fontSize:12, fontWeight:600, border:'1.5px solid #E2E8F0', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Ver detalle</button>
                    </>}
                    {ev.motivo === 'EVIDENCIA_INSUFICIENTE' && <>
                      <button onClick={() => navigate('/guardia/revision-manual', { state:{ eventoId:ev.id } })} style={{ padding:'10px 16px', borderRadius:9, background:'#0D5CCF', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>ℹ Ver detalle</button>
                      <button onClick={() => navigate('/guardia/contingencia', { state:{ eventoId:ev.id } })} style={{ padding:'10px 16px', borderRadius:9, background:'#F2851F', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>⚠ Contingencia</button>
                    </>}
                    {ev.motivo === 'CONDUCTOR_PRESENTA_PASE' && <>
                      <button onClick={() => navigate('/guardia/pase-rapido', { state:{ eventoId:ev.id, placa:ev.placa } })} style={{ padding:'10px 16px', borderRadius:9, background:'#0D5CCF', color:'#fff', fontSize:12, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>🔓 Validar pase rápido</button>
                      <button onClick={() => navigate('/guardia/revision-manual', { state:{ eventoId:ev.id } })} style={{ padding:'9px 16px', borderRadius:9, background:'#F8FAFC', color:'#374151', fontSize:12, fontWeight:600, border:'1.5px solid #E2E8F0', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Ver detalle</button>
                    </>}
                  </div>
                </div>
                {ev.motivo === 'CONDUCTOR_NO_AUTORIZADO' && (
                  <div style={{ background:'#FFF8F8', borderTop:`1px solid ${cc.border}`, padding:'9px 22px', fontSize:11, color:'#991B1B' }}>
                    ⚠ Aprobación rápida no disponible — revise el detalle antes de decidir
                  </div>
                )}
              </div>
            );
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999 }}>
            <div style={{ background:'#fff', borderRadius:16, border:'1px solid #FECACA', padding:28, width:420, boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16 }}>
                ...
                <button onClick={() => setModalDenegar(null)} style={{ background:'none', border:'none', fontSize:20, cursor:'pointer', color:'#888' }}>✕</button>
              </div>
              <div style={{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:10, padding:'14px 18px', marginBottom:16 }}>
                <div style={{ fontFamily:'"Exo 2",sans-serif', fontSize:22, fontWeight:800, color:'#991B1B', letterSpacing:2 }}>{modalDenegar.placa}</div>
                <div style={{ fontSize:12, color:'#C62828', marginTop:4 }}>{modalDenegar.movimiento} · {motivoLabel[modalDenegar.motivo]}</div>
              </div>
              <div style={{ fontSize:13, color:'#555', marginBottom:20 }}>¿Confirmas que deseas <strong style={{ color:'#C62828' }}>denegar este acceso</strong>? El evento se cerrará con decisión DENIED.</div>
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => setModalDenegar(null)} style={{ flex:1, padding:11, borderRadius:8, background:'#fff', color:'#555', fontSize:13, fontWeight:500, border:'1.5px solid #E2E8F0', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Cancelar</button>
                <button onClick={() => confirmarDenegar(modalDenegar)} style={{ flex:1, padding:11, borderRadius:8, background:'#C62828', color:'#fff', fontSize:13, fontWeight:700, border:'none', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>✕ Confirmar denegación</button>
              </div>
            </div>
          </div>
        )}
