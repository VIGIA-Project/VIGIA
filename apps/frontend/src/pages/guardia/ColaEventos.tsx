...
// Interface adaptada para la UI basada en la Alerta real
interface UIMappedEvent {
  id: string;
  placa: string;
  timeAgo: string;
...
// Función auxiliar para mapear la alerta a la estructura esperada por la UI
const mapAlertToUIEvent = (alert: Alert): UIMappedEvent => {
  ...
  return {
    id: alert.id,
    placa: 'PENDIENTE', // No disponible en Alert directo sin join
    timeAgo: `hace ${diffMinutes} min`,
    timeAgoUrgent: diffMinutes > 5,
    direction: 'ENTRADA', // Valor por defecto
    timestamp: date.toLocaleTimeString(),
    alertTitle: alert.alertTitle,
    alertDescription: alert.alertDescription,
    alertType: alert.alertType,
    vehiculo: 'Vehículo en verificación',
    propietario: 'Propietario en verificación',
  };
};
...
export const ColaEventosPage: React.FC = () => {
  ...
  const [selectedEvent, setSelectedEvent] = useState<UIMappedEvent | null>(null);
  ...
  const mappedEvents = alerts.map(mapAlertToUIEvent);
  ...
  const filteredEvents = mappedEvents.filter((event) => {
    ...
    // Basic sort logic using timestamp string as fallback
    if (sortOrder === 'MasAntiguo') return new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime();
    return new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime(); // MasReciente
  });
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible">
          {isLoading ? (
            <Box sx={{ p: 4, textAlign: 'center' }}>
               <Typography sx={{ color: vigiaColors.textSecondary }}>Cargando eventos...</Typography>
            </Box>
