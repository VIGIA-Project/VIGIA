    ...
    <DashboardTemplate rol="GUARD" pageTitle="Inicio del turno">
      <Box sx={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20 }}>
          ...
          <div style={{ fontSize:12, color:'#6B7280', background:'#fff', borderRadius:8, padding:'8px 16px', border:'1px solid #E2E8F0' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:20 }}>
          {[
            { label:'Pendientes entrada', value:'3', color:'#F2851F', sub:'Requieren decisión', bg:'#FFF3E0', icon:'⏱', ruta:'/guardia/cola-eventos' },
            { label:'Pendientes salida',  value:'4', color:'#C62828', sub:'2 salidas riesgosas', bg:'#FEE2E2', icon:'⚠', ruta:'/guardia/cola-eventos' },
            { label:'Tiempo promedio',    value:'1:45', color:'#19D6C4', sub:'min por resolución', bg:'#E0FDF4', icon:'⏰', ruta: null },
            { label:'Eventos del turno',  value:'23',  color:'#0D5CCF', sub:'12 entradas · 11 salidas', bg:'#EEF2FF', icon:'📊', ruta:'/guardia/historial' },
          ].map((k) => (
            <div
              key={k.label}
              onClick={() => k.ruta && navigate(k.ruta)}
              style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', padding:20, borderTop:`3px solid ${k.color}`, boxShadow:'0 2px 8px rgba(10,47,134,0.06)', cursor: k.ruta ? 'pointer' : 'default', transition:'all 0.15s' }}
              onMouseEnter={e => { if(k.ruta) (e.currentTarget as HTMLElement).style.boxShadow='0 4px 16px rgba(10,47,134,0.12)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow='0 2px 8px rgba(10,47,134,0.06)'; }}
            >
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', boxShadow:'0 2px 8px rgba(10,47,134,0.06)' }}>
            ...
            {[
              { placa:'PCB-1234', tipo:'SALIDA',  motivo:'CONDUCTOR NO AUTORIZADO', color:'#C62828', bg:'#FFF8F8', t:'1 min' },
              { placa:'ABC-5678', tipo:'ENTRADA', motivo:'VEHICULO_NO_REGISTRADO',  color:'#F2851F', bg:'#fff',    t:'3 min' },
              { placa:'XYZ-9012', tipo:'SALIDA',  motivo:'EVIDENCIA_INSUFICIENTE',  color:'#EDB200', bg:'#fff',    t:'5 min' },
            ].map((e) => (
              <div
                key={e.placa}
                onClick={() => navigate('/guardia/cola-eventos')}
                style={{ padding:'12px 20px', display:'flex', alignItems:'center', gap:12, borderBottom:'1px solid #F8F8F8', background:e.bg, cursor:'pointer', transition:'background 0.15s' }}
              ...
              >
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:2 }}>
                    <span style={{ fontSize:9, fontWeight:600, padding:'2px 7px', borderRadius:20, background: e.tipo==='ENTRADA'?'#E3F2FD':'#EDE7F6', color: e.tipo==='ENTRADA'?'#1565C0':'#4527A0' }}>{e.tipo}</span>
                    <span style={{ fontSize:11, fontWeight:600, color:e.color }}>{e.motivo}</span>
                  </div>
                </div>
                <div style={{ fontSize:10, color:e.color, fontWeight:600, whiteSpace:'nowrap' }}>{e.t}</div>
              </div>
            ))}
            <div
              onClick={() => navigate('/guardia/cola-eventos')}
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', boxShadow:'0 2px 8px rgba(10,47,134,0.06)' }}>
            ...
            {[
              ...
              <div
                key={i}
                onClick={() => navigate('/guardia/alertas')}
                style={{ padding:'12px 20px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid #F8F8F8', opacity: a.leida?0.6:1, cursor:'pointer', transition:'background 0.15s' }}
                onMouseEnter={e => (e.currentTarget.style.background='#F8FAFF')}
                onMouseLeave={e => (e.currentTarget.style.background='transparent')}
              >
                ...
                <div style={{ fontSize:10, color:'#94A3B8', whiteSpace:'nowrap' }}>{a.t}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', boxShadow:'0 2px 8px rgba(10,47,134,0.06)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, padding:'16px 18px 10px' }}>
              {[
                {v:'12',l:'Entradas',      c:'#2E7D32', bg:'#F0FFF4', border:'#BBF7D0'},
                {v:'11',l:'Salidas',       c:'#4527A0', bg:'#EDE7F6', border:'#D1C4E9'},
                {v:'2', l:'Contingencias', c:'#E65100', bg:'#FFF3E0', border:'#FFE0B2'},
              ].map(x=>(
                <div key={x.l} style={{ textAlign:'center', padding:'14px 8px', background:x.bg, borderRadius:10, border:`1px solid ${x.border}` }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, padding:'0 18px 16px' }}>
              {[
                {v:'16',l:'Automáticos', c:'#19D6C4'},
                {v:'5', l:'Manuales',    c:'#0D5CCF'},
              ].map(x=>(
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', boxShadow:'0 2px 8px rgba(10,47,134,0.06)' }}>
            ...
            {[
              { dot:'#4CAF50', txt:'Acceso autorizado · PBW-1234 · ENTRADA · Garita Norte', t:'2 min' },
              ...
              { dot:'#F2851F', txt:'Contingencia · PBA-5544 · Fallo OCR registrado',         t:'22 min' },
            ].map((a,i)=>(
              <div
                key={i}
                onClick={() => navigate('/guardia/historial')}
                style={{ padding:'11px 20px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid #F8F8F8', cursor:'pointer', transition:'background 0.15s' }}
                onMouseEnter={e => (e.currentTarget.style.background='#F8FAFF')}
                onMouseLeave={e => (e.currentTarget.style.background='transparent')}
              >
