...
// Usamos el mapping desde AccessEvent real
const mapEventToHistorial = (event: AccessEvent): HistorialEvento => {
  const d = new Date(event.createdAt);
  
  let grupo: 'hoy' | 'ayer' | 'anterior' = 'anterior';
  const hoy = new Date();
  if (d.getDate() === hoy.getDate() && d.getMonth() === hoy.getMonth() && d.getFullYear() === hoy.getFullYear()) {
    grupo = 'hoy';
  } else if (d.getDate() === hoy.getDate() - 1) {
    grupo = 'ayer';
  }

  let resultado: ResultadoEvento = 'DENEGADO';
  if (event.decision === 'GRANTED') {
    resultado = event.direction === 'ENTRADA' ? 'ENTRADA_PERMITIDA' : 'SALIDA_PERMITIDA';
  }
  ...
  return {
    ...
    placa: event.licensePlate || 'SIN_PLACA',
    resultado,
    puntoAcceso: 'Garita Principal', // Mock por ahora
    persona: event.personId ? 'Persona Registrada' : 'Desconocido', // requeriria join
    grupo
  };
};
...
const HistorialPage: React.FC = () => {
  ...
  const filtrados = useMemo(() => {
    ...
    const mapped = events.map(mapEventToHistorial);
  ...
  }, [events, vehiculo, resultado, busqueda]);
