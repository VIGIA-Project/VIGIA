...
const PersonaDetallePage: React.FC = () => {
  ...
  const handleRevoke = async (revokeId: string) => {
    try {
      ...
      // Forzamos el refresco inmediato de la query para actualizar los estados reactivamente
      await autorizacionesQuery.refetch();
    } catch {
                    ...
                    <Box sx={{ p: 2.25, borderRadius: vigiaRadius.md, backgroundColor: '#FEF3C7', border: '1px solid #F59E0B' }}>
                      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Button
                              variant="contained"
                              onClick={() => navigate(`/propietario/personas/${id}/biometria`)}
